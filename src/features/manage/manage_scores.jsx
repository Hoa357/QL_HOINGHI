...
const ManageRegistrationsPage = () => {
  ...
  const generateQRCode = async () => {
    ...
    const confirm = window.confirm(
      "Bạn có chắc chắn muốn tạo mã QR mới? Mã sẽ có hiệu lực trong 10 phút."
    );
    ...
    if (confirm) {
      try {
        ...
        const token = `${Date.now()}-${Math.random()
          .toString(36)
          .substring(2)}`;
        const expires = new Date(Date.now() + 10 * 60 * 1000); // Hết hạn sau 5 phút
